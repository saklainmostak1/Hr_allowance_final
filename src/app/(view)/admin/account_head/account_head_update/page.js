...
const AccountHeadUpdate = ({ id }) => {
    ...
    const { data: accountHeadTypeSingle, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/Admin/account_head/account_head_all/${id}`);
            const data = await res.json();
            return data;
        }
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/Admin/account_head/account_head_edit/${id}`, {
                method: 'POST',
                headers: {
